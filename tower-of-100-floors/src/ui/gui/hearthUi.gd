...
func update_actual_hearts(action_name: String, action_count: int) -> void:
	match action_name:
		"damage":
			_unstack(action_count)
		"heal":
			_stack(action_count)

func _stack(loop_count: int = 1) -> void: #Empilhar
...
func _unstack(loop_count: int = 1) -> void:
	for i in range(loop_count):
